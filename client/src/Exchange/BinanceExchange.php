...
class BinanceExchange
{
    ...
    public function getOpenOrder($symbol)
    {
        $orders = $this->api->openOrders();
        $formatOrders = $this->formatOpenOrders($orders);
        $symbol = $this->getSymbolInfomation($symbol);
        foreach ($formatOrders as $formatOrder) {
            if ($formatOrder['symbolName'] == $this->renderSymbolNamebySymbol($symbol)) {
                return $formatOrder;
            }
        }
        return false;
