...
//    }
  ...
  public function process(Activity $activity, $userId) {
    ...
    $command->addOjbect($data, $exchange);
  ...
  public function cancelOrder($uid, $oderIndex)
  {
    $exchange = $this->getExchange('bn', $uid);
    $orders = $exchange->cancelOrder($oderIndex);
    return $this->templateService->renderOpenOrders($orders);
  }
  ...
  public function processUserCommand($text, $uid) {
    if ($args = explode(" ", $text)) {
      switch ($args[0]) {
        case 'o':
          return $this->getOpenOrders($uid);
        case 'bl':
          return $this->getBalanceInfo($uid);
        case 'cancel':
          return $this->cancelOrder($uid, $args[1]);
      }


      if (count($args) == 1) {
        return $this->getPriceInfo($args[0], $uid);
      }
    }
  }
  ...
  public function detach($text = null, $uid) {
    ...
    if (substr($text, 0, 1) !== '/') {
      $this->messages->log('Invalid command, command is required /');
      return $this->messages->getMessages();
    }
    $text = ltrim($text, '/');
...
//      $this->updateStatus($activity);
  ...
  public function processCommand($text, $uid) {
    ...
    $command->addOjbect($args, $exchange);
