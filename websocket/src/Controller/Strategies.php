...
//    }
   ...
   * @return int
   */
  public function bowhead_rsi_macd($pair, $data, $return_full=false, $text = '')
  {
    $indicators = new Indicators();
   ...
   * @return int
   */
  public function bowhead_stoch_adx($pair, $data, $return_full=false, &$text = null)
  {
    $indicators = new Indicators();
    ...
    $adx   = $indicators->adx($pair, $data, 14, $text);
//    $stoch = $indicators->stoch($pair, $data);
...
//    } elseif ($adx == 1 && $stoch > 0 && $bullish) {
   ...
   * @return int
   */
  public function bowhead_double_volatility($pair, $data, $return_full=false, &$text = '')
  {
    ...
    $text .= ' sma20 high: ' . $sma20_high;
    $text .= ' sma20 low: ' . $sma20_low;
    $text .= ' sma5 high: ' . $sma5_high;
    $text .= ' rsi: ' . $rsi;
   ...
   * @return int
   */
  public function bowhead_bband_rsi($pair, $data, $return_full=false, &$text = '')
  {
    $indicators = new Indicators();

    $rsi    = $indicators->rsi($pair, $data, 11, $text);
    $bbands = $indicators->bollingerBands($pair, $data, 20, 2, 2, $text);
    if ($rsi > -1 && $bbands == -1) {
      $return['side']     = 'short';
   ...
   *  @return int
   */
  public function bowhead_5th_element($pair, $data, $return_full=false, &$text = '')
  {
    $indicators = new Indicators();
