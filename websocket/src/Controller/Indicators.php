...
class Indicators
{
  ...
  public function bollingerBands($pair='BTC/USD', $data=null, $period=10, $devup=2, $devdn=2, &$text = '')
  {
    if (empty($data)) {
      $data = $this->getRecentData($pair);
    }
    ...
    $text .= ' bb: up: ' . array_pop($upper) . ' current: ' . $current . ' down: ' . array_pop($lower);
   ...
   */
  public function adx($pair='BTC/USD', $data=null, $period=14, &$text = null)
  {
    if (empty($data)) {
      $data = $this->getRecentData($pair);
    }
