...
class BinanceController extends Controller
{
    ...
    public function changeStrToUnixtime($date)
    {
        return [
            'from' => strtotime($date) * 1000,
            'to' => strtotime('+2 day', strtotime($date)) * 1000,
            'date' => date('d_m_Y', strtotime($date))
        ];
    }

    public function takeCandleFile($date)
    {
        $time = $this->changeStrToUnixtime($date);
        $fileName = "{$time['date']}.json";
        if (self::existFile($fileName)) {
            return self::readFile($fileName);
        }

        $candles = $this->binance->candlesticks('ADAUSDT', "1h",48, $time['from'], $time['to']);
    ...
    public function testWebsocket()
    {
        ...
        $candles = $this->takeCandleFile('-4 day');
